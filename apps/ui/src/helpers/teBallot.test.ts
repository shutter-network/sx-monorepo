...
import { buildTeBallotEnvelope, buildTeWeightedBallotEnvelope, pseudonymFor } from './teBallot';
...
describe('buildTeWeightedBallotEnvelope — input validation', () => {
  const BASE_ARGS = {
    ...
    proposalId: '0x' + '22'.repeat(32),
    mpk: '0x' + 'ab'.repeat(96),
    config: { variant: 'A' as const, mode: 'exact' as const, budget: 100, numCandidates: 3 },
    choice: { '1': 60, '2': 40 }
  };

  it('rejects Variant B', async () => {
    await expect(
      buildTeWeightedBallotEnvelope({ ...BASE_ARGS, config: { ...BASE_ARGS.config, variant: 'B' as any } })
    ).rejects.toThrow('only Variant A exact');
  });

  it('rejects atMost mode', async () => {
    await expect(
      buildTeWeightedBallotEnvelope({ ...BASE_ARGS, config: { ...BASE_ARGS.config, mode: 'atMost' as any } })
    ).rejects.toThrow('only Variant A exact');
  });

  it('rejects empty choice (zero total weight)', async () => {
    await expect(
      buildTeWeightedBallotEnvelope({ ...BASE_ARGS, choice: { '1': 0, '2': 0 } })
    ).rejects.toThrow('weights sum to zero');
  });

  it('rejects malformed mpk hex (bad on-curve bytes)', async () => {
    await expect(
      buildTeWeightedBallotEnvelope({ ...BASE_ARGS, mpk: '0x' + '00'.repeat(96) })
    ).rejects.toThrow();
  });
});
...
describe('buildTeWeightedBallotEnvelope — envelope shape (real crypto)', () => {
  const CONFIG = { variant: 'A' as const, mode: 'exact' as const, budget: 100, numCandidates: 3 };

  it('returns a valid envelope with all required fields', async () => {
  ...
  it('electionId matches proposalId bytes', async () => {
  ...
  it('produces numCandidates ciphertexts regardless of how many choices are specified', async () => {
  ...
  it('two votes with different splits produce different ciphertexts', async () => {
    const base = {
      voter: '0x' + '11'.repeat(20),
    ...
    const a = await buildTeWeightedBallotEnvelope({ ...base, choice: { '1': 60, '2': 40 } });
    const b = await buildTeWeightedBallotEnvelope({ ...base, choice: { '1': 40, '2': 60 } });
