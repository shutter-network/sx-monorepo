...
1. **Create** — hub writes the row with `te_mpk = NULL`; auto-DKG drives each keyper through DKG and
   the hub stores `te_mpk` + `te_committee_pks` once it has `t+1` matching submissions.
...
3. **Tally** — on `closed`, the sequencer builds the per-candidate homomorphic sum, persists
   ...
   shares, it Lagrange-combines, BSGS-recovers, and writes `proposal.scores`.
...
- **`recoverTally: candidate X share from keyper Y failed verification`** → the proposal's committee
  ...
  409 `keyper_changed_submission`), set `proposals.te_mpk=NULL`, then let auto-DKG pick it up. Also
  `DELETE FROM te_decryption_shares WHERE proposal_id=?` before re-tallying. The auto-DKG coordinator
  skips a proposal after 5 failures until keypers restart.
